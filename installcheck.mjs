...
//
// It lived as three byte-identical copies in tools/, and the argument for that
// was that the logic is generic and names no package, so copying costs nothing
// but a comment. That accounted for the copies that existed and not for the one
// that did not: wochenwerk had neither the file nor the script, and so had
// neither gate, for as long as the arrangement stood. Three copies did not
// drift - they failed to reach the fourth product, which is a different failure
// and one the copy argument does not answer.
//
// The second half was wrong too. There *was* something to keep in step besides
// the comment: the CI wiring, which is not in this file and drifted furthest of
// all. bildhaft ran it only when recording baselines, mitreden nowhere,
// vorlaut-editor only before its tests. Copying a file does not copy a gate.
//
// So it sits beside pins.js, which has been invoked out of node_modules since
// it was written and proves the arrangement. The two ask opposite questions of
// the same packages: this one whether the install matches the pin, pins.js
// whether the pin has fallen behind what has been published.

import { readFileSync } from "node:fs";
import { join, resolve } from "node:path";

// The directory npm was run in, not this file's own - the whole point is that
// one copy answers for every product, and out of node_modules a path relative
// to this file lands in node_modules/@lautstark. pins.js resolves the same way.
